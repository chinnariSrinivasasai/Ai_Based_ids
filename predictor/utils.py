...
# TensorFlow backend used by keras; keep a reference to the default graph so
# we can make predictions from any thread/request without running into the
# classic "Tensor ... is not an element of this graph" error.
import tensorflow as tf

# Use SAME keras as training, but reference through tf.keras to ensure the
# model is tied to the TensorFlow graph that we capture below.
...
model = None
# reference to the tf graph the model was created/loaded under
graph = None
explainer = None
...
# ===============================
# Load LSTM Model (Matches Training Exactly)
# ===============================
def get_model():
    global model
    ...
    global graph
    if model is None:
        model_path = os.path.join(settings.ML_ASSETS_DIR, 'lstm_weights.hdf5')
        ...
        # capture the current default graph so we can reuse it later
...
def make_prediction(input_string):
    try:
        ...
        model = get_model()
        # ensure we are executing on the same graph the model was built on
        global graph
        if graph is not None:
            with graph.as_default():
                prediction_prob = model.predict(input_array, verbose=0)
        else:
            prediction_prob = model.predict(input_array, verbose=0)
        ...
        shap_plot = generate_shap_plot(model, input_array)
        lime_html = generate_lime_explanation(model, input_array)
...
# SHAP Explanation
# ===============================
def generate_shap_plot(model, input_array):
...
# LIME Explanation
# ===============================
def generate_lime_explanation(model, input_array):
...
def load_training_history():
    path = os.path.join(settings.ML_ASSETS_DIR, 'lstm_history.pckl')
